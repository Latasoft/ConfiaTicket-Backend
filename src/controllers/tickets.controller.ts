...
/* ============================================================
 *  Webpay helper (igual que en payments.controller.ts)
 * ==========================================================*/
function tbkTx() {
...
/* ====================== helpers de dinero ====================== */
/**
 * Determina el monto neto para el organizador.
 * Prioriza Payment.netAmount si está seteado; si no, usa amount - applicationFeeAmount.
 * Fallback a amount si nada de lo anterior está disponible.
 */
function computeNetForOrganizer(payment: {
  amount: number;
  netAmount: number | null | undefined;
  applicationFeeAmount: number | null | undefined;
}): number {
  if (typeof payment.netAmount === "number" && payment.netAmount > 0) return payment.netAmount;
...
/** Genera una clave de idempotencia persistente para un payout nuevo */
...
/** Verifica que el ConnectedAccount esté listo para recibir pagos */
...
export async function adminApproveTicket(req: Request, res: Response) {
  try {
    ...
    const ensureAndTriggerPayout = async (txPay: typeof p) => {
      ...
      if (!payout) {
        const net = computeNetForOrganizer({
          amount: txPay.amount,
    ...
    // Captura en Webpay
    const tx = tbkTx();
    const cap = await tx.capture(p.token, p.buyOrder, p.authorizationCode, p.amount);
...
export async function refreshReservationPayment(req: Request, res: Response) {
  try {
    ...
    if (p?.token) {
      try {
        const tx = tbkTx();
        const st: any = await tx.status(p.token);
