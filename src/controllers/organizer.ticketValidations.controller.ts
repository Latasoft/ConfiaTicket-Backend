// src/controllers/organizer.ticketValidations.controller.ts
 ...
 * 
 * Lista las validaciones de tickets del organizador (OWN y RESALE)
 * Parámetros query:
 * - eventId?: number
 ...
 */
export async function listOrganizerValidations(req: Request, res: Response) {
  try {
    ...
    console.error('❌ Error al listar validaciones del organizador:', error);
 ...
 * 
 * Obtiene detalles de una validación específica del organizador
 * Query param: type ('OWN' | 'RESALE')
 */
export async function getOrganizerValidationDetails(req: Request, res: Response) {
  try {
    const user = (req as any).user as Authed;
